import type { Anime as MALAnime, AnimeStatus } from "./mal/types";
...
export function mapMALAnime(mal: MALAnime): Anime {
  ...
  return {
    ...
    source: mal.source ?? "",
    duration: mal.average_episode_duration
    ...
    listKey: mal.my_list_status?.status
      ? mapListStatus(mal.my_list_status.status)
      : undefined,
    watchedEps: mal.my_list_status?.num_episodes_watched ?? 0,
  };
}
...
export function unmapListStatus(key: ListKey): AnimeStatus {
  switch (key) {
    case "watching":
      return "watching";
    case "completed":
